...
*** bash
 : stow -t $HOME bash -D --adopt
*** zsh
 : stow -t $HOME zsh -D --adopt
*** vim
: stow -t $HOME vim  -D --ignore=^[^\.]+\..*vim.* --adopt
*** ideavimrc
: stow -t $HOME ideavimrc -D --ignore=_ideavimrc --ignore=idea_actionlist --adopt
*** git
: stow -t $HOME git -D --adopt
*** tmux
: stow -t $HOME tmux -D --adopt
need xclip
: tmux show-options -g -s set-clipboard
*** brew
: stow -t $HOME brew -D --adopt
=brew/.Brewfile= 软链到 =~/.Brewfile=，=brew bundle --global= 直接读取。
*** nodejs
: stow -t $HOME nodejs -D --adopt
*** ctags
: stow -t $HOME ctags -D --adopt
*** tar
: stow -t $HOME tar -D --adopt
*** Xresources
: stow -t $HOME Xresources -D
*** mpv
: stow -t $HOME/ mpv -D
*** ghostty
: stow -t $HOME/ ghostty -D
*** i3
: stow -t $HOME/ i3 -D
*** awesome
...
下表是本仓库的**现役部署命令**（上面各节的 =stow -D --adopt= 是首次收集配置时的
一次性步骤，日常部署不要 =-D=）。顶层目录各是一个包，=docs/= 是文档不是包。

: stow -t $HOME --ignore='\.example$' bash zsh git tmux nodejs ctags tar mpv ghostty alacritty aria2 mail sbcl pip
: stow -t $HOME Xresources X11 wayland xmonad i3 awesome rofi   # Linux 桌面/窗口管理器
: stow -t $HOME brew                                            # → ~/.Brewfile，brew bundle --global 读
: stow -t $HOME vim --ignore='^[^\.]+\..*vim.*' --ignore='\.example$'
: stow -t $HOME ideavimrc --ignore='_ideavimrc' --ignore='idea_actionlist'
: stow -t $HOME/.config/hypr   hypr     # 单文件 hyprland.conf → ~/.config/hypr/
: stow -t $HOME/.config/mpd    mpd
: stow -t $HOME/.config/ranger ranger
: stow -t $HOME/.config/sway   sway

** 机器差异管理（.local 机制）

本仓库使用单一 =master= 主干，机器差异（工作机 vs 个人机）不用分支，用每机 =*.local= 文件表达：

- 追加型（vim/zsh/bash）：复制 =xxx/.xxx.local.example= 到本机 =~/.xxx.local= 填值，主配置末尾自动 =source=。
- 整文件型（git 身份）：用 git 原生 =includeIf=，见 =git/.gitconfig.local.example=。

标准模板与规则见 =docs/machine-local.md=。新增机器 = clone + 复制 .local.example = 即可，不切分支、不 rebase。
